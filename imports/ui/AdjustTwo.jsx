...
const AdjustTwo = ({ setData, setStep,datta,fetch}) => {
  ...
  const renderLegalProfessionals=()=>{
      Meteor.call('showLegalProf',(err,res)=>{
          setLegalProfessionals(res);
      })
  }
  useEffect(()=>{
      fetch()
      renderLegalProfessionals();
  },[])
  const { register, handleSubmit, errors } = useForm({
    resolver: yupResolver(Schema2),
    defaultValues: {
     legalprof1:namelegalprof,
      infoB: datta.infoB,
      vousB: datta.vousB,
  ...
  return (
    <div>
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <label>Choose the legal professional </label>
          <select name="legalprof1" ref={register} className="form-select mb-5" >
          {legalprofessionals.map((legalprof)=>{
            return(
              <SelectLegalProfessionals
                  key={legalprof._id}
                  legalprof={legalprof}
                  fetch={renderLegalProfessionals}
              />
          ); 
            })}
            </select>
